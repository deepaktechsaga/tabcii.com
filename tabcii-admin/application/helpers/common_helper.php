...
function get_category_name($id)
{
        $ci=& get_instance();
        $ci->load->database(); 
        $sql = "select category from tbl_category where id =$id"; 
        $query = $ci->db->query($sql);
        $data = $query->row();        
        if($query->num_rows() > 0)
        {
...
function get_class_tags($p_parent_cat_id, $tag_cat_id)
{
        $ci=& get_instance();
        $ci->load->database(); 
        $sql = "SELECT tg.id,tg.name FROM `tags` tg join `tags_category` tc on tg.`tag_category_id` = tc.id WHERE tc.product_parent_cat_id= $p_parent_cat_id and tg.tag_category_id = $tag_cat_id"; 
        $query = $ci->db->query($sql);
