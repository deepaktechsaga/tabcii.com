...
class Bus extends CI_Controller {
	...
	public function manage_log()
	{
	    if($this->session->userdata('is_admin_logged'))
		{
			...
			$data["log_data"] = $this->Ve_model->get_rows_limit('tabcii_bus_booking','id','desc',$config["per_page"], $page); //print_r($data["user_data"]); die();		
			$data["total_rows"] =  $query->num_rows(); 

			$this->load->view('admin/header');		
			$this->load->view('admin/bus/manage-booking-log',$data);
	...
	public function view_log()
	{
	    if($this->session->userdata('is_admin_logged'))
		{
			...
			$this->load->view('admin/header');		
			$this->load->view('admin/bus/view-booking-log',$data);
			$this->load->view('admin/footer'); 			
		}
		else
		{
